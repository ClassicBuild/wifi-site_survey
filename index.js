class GameScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image('remove','assets/images/remove.png');
        this.load.image('connection','assets/images/connection.png');
    }
    ...
    create() {
        ...
        // this.add
        //     .text(16, 16, `${document.getElementById('username').value} \nArrow keys to move\nPress "S" to show stats menu`, {
        //         font: "18px Arial",
        //         fill: "#000000",
        //         padding: {
        //             x: 20,
...
class Phase1MenuScene extends Phaser.Scene {
    ...
    create() {
        ...
        let info = this.add.text(320, 620, `\n\n\n\nScore : ${ this.score} \nInterference Objects Found : ${ this.interferenceObjectsFound}`, {
            font: '24px Arial',
            fill: '#000000'
        }).setInteractive();
        ...
        //menu background
        var radarbg = this.add.rectangle(155, 722, 310, 158, 0x000000);
        radarbg.depth = 99;
        this.add.rectangle(305, 722, 1250, 158, 0x000000);
        this.add.rectangle(305, 722, 1180, 148, 0xFFFFFF);


        // Help text that has a "fixed" position on the screen
        this.add
            .text(320, 665, `${document.getElementById('username').value} \nArrow keys to move.   Press "S" to show stats menu.`, {
                font: "18px Arial",
                fill: "#000000"
            })
            .setScrollFactor(0)
        ...
        var nextPhaseButton = this.add.rectangle(785, 750, 110, 25, 0xFFFFFF).setInteractive();
        nextPhaseButton.setStrokeStyle(2, 0x000000, 3);
        nextPhaseButton.depth = 99;
        //text for the next phase button
        let nextPhaseText = this.add.text(740, 740, `Next Phase`, {
            font: '18px Arial',
            fill: '#000000'
        });
        ...
        this.confirmPhaseButton = this.add.rectangle(785, 750, 140, 35, 0xFFFFFF).setInteractive();
        this.confirmPhaseButton.setStrokeStyle(2, 0xFF0000, 3);
        this.confirmPhaseButton.depth = 99;
        this.confirmPhaseButton.visible = false;

        //text for the confirm button
        this.confirmPhaseText = this.add.text(725, 740, `Click to confirm`, {
            font: '18px Arial',
            fill: '#000000'
        });
        ...
        nextPhaseButton.on('pointerdown', () => {
            nextPhaseButton.fillColor = 0xAAAAAA;
        }, this);
        nextPhaseButton.on('pointerup', () => {
            nextPhaseButton.fillColor = 0xFFFFFF;
            nextPhaseButton.visible = false;
            nextPhaseText.visible = false;
            confirmPhase(this);
        }, this);
    ...
    update(time, delta) {
        ...
        if (!this.toggle) {
          this.graphics.lineStyle(1.5, 0x22FFaa);
            var y = (10 * this.speed) * Math.sin(this.angle) + 600;
            this.points[this.iterator] = new Phaser.Geom.Point(3.5 * this.iterator + 10, y + 120);
            if (this.iterator >= 1) {
...
class Phase2MenuScene extends Phaser.Scene {
    ...
    create() {
        ...
        //  Our Text object to display the Score
        let info = this.add.text(10, 720, `Coverage Score :% ${0} \nAntennas placed : ${0}`, {
            font: '24px Arial',
            fill: '#000000'
        }).setInteractive();
        ...
        var nextPhaseButton = this.add.rectangle(770, 750, 80, 25, 0xFFFFFF).setInteractive();
        nextPhaseButton.setStrokeStyle(3, 0x000000, 1);
        nextPhaseButton.depth = 99;
        let nextPhaseText = this.add.text(742, 740, `Submit`, {
            font: '16px Arial',
            fill: '#000000'
        });
        nextPhaseText.depth = 100;

        this.confirmPhaseButton = this.add.rectangle(770, 750, 100, 35, 0xFFFFFF).setInteractive();
        this.confirmPhaseButton.setStrokeStyle(3, 0x000000, 1);
        this.confirmPhaseButton.depth = 99;
        this.confirmPhaseButton.visible = false;

        this.confirmPhaseText = this.add.text(742, 740, `End Game`, {
            font: '16px Arial',
            fill: '#000000'
        });
        this.confirmPhaseText.depth = 100;
        ...
        // Phase 2 Buttons
        this.addAntennaBtn = this.add.rectangle(808, 715, 30, 30, 0x0dc040).setInteractive();
        this.removeAntennaBtn = this.add.rectangle(768, 715, 30, 30, 0xdd1010).setInteractive();
        this.toggleInterferenceZonesBtn = this.add.rectangle(728, 715, 30, 30, 0x1210CD).setInteractive();
